// LEMSv2 Notes:
// Modified from code originally provided by Stroud Water Research Center (See below)
// Modified for Arduino Zero. Untested with other Arduinos/Sensors
// Currently in communication with Stroud. If changes get approved, will start using official version
...
#include "SDI12_Zero.h"						// 0.1 header file for this library
...
// Returns 1 i val as an odd number of bits set
// i.e., will an additional 1 or 0 make the final number of 1's even?
// Replacement for parity_even_bit found in <avr/parity.h>. See
// 	www.nongnu.org/avr-libc/user-manual/parity_8h_source.html
// Code courtesy of @TypeIA on stack overflow: http://stackoverflow.com/a/21618038
static inline uint8_t parity_even_bit(uint8_t val){
	val ^= val >> 4;
	val ^= val >> 2;
	val ^= val >> 1;
	return (val) & 1;
...
void SDI12::setState(uint8_t state){
  if(state == HOLDING){
    ...
    digitalWrite(_dataPin,LOW);		// I don't know why this works and pinMode(_dataPin,OUTPUT) doesn't. I assume it's an impedance thing
  ...
  if(state == LISTENING) {
    ...
    attachInterrupt(_dataPin, handleInterrupt, CHANGE);
  }
